...
from app.dependencies import ensure_cluster_writable, get_db_session, get_writable_db_session
...
from app.services import etcd_quorum as etcd_quorum_service
...
async def etcd_quorum(
    ...
    try:
        return await etcd_quorum_service.evaluate_quorum(session)
    except Exception as exc:  # noqa: BLE001
...
async def reconcile_etcd_quorum(
    ...
    if not dry_run:
        await ensure_cluster_writable(session)
    try:
        return await etcd_quorum_service.reconcile_quorum(session, dry_run=dry_run)
    except HTTPException:
        raise
    except Exception as exc:  # noqa: BLE001
        raise HTTPException(status_code=503, detail=f"Unable to reconcile etcd quorum: {exc}") from exc
...
async def restore_snapshot(
    ...
    except snapshot_service.SnapshotRestoreRejected as exc:
